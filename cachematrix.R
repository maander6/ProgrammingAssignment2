...
## within the makeCacheMatrix() function.  These functions (10 allow 
...
## function, and the value of the inverse matrix is cached using the setInverse()
## function.
##
...
## The cacheSolve() function checks to see if the inverse of the matrix has already 
## been calculated and its value saved to cached memory.  If so, the cached value is returned
## and the function exited.
##
## If not, then the function checks to see if the invers can be solved by checking to see
## if it is square, and then checking if the determinant of the matrix is non-zero.  
## Because of the precision of the computation, a determinant value of less that 1e-10 is
## assumed to be equal to zero.  These 2 conditions are required for an inverse of a matrix 
## to be calculated.  Note the instructions said to assume that the input matrix is invertible
## these error checks were introduced  to be complete.
##
## Once the inverse matrix has been calculated, the value of the inverse matrix is saved to the
## cache memory for rapid recall in subsequent calls to the function, saving computational time
##
...
cacheSolve <- function(x, ...) {        ## Return a matrix that is the inverse of 'x'
	...
	if (abs(detM) < 1e-10) {        ## Although the instructions said 
					...
					## and serves as an error trap if the inverse
        ...
        mInverse        ## This statement simply returns the inverse matrix to the 
                        ## calling environment.
